...
contract("MultiSigWalletWithDailyLimitFactory", (accounts) => {
    ...
    it("Multisig Factory", async () => {
        ...
        const multisigWalletAddressConfirmation = await factoryInstance.instantiations(
            accounts[0],
            walletCount.sub(1).toNumber()
        );
        ...
        await new Promise((resolve, reject) =>
            web3.eth.sendTransaction({ to: walletAddress, value: deposit, from: accounts[0] }, (e) =>
                e ? reject(e) : resolve()
            )
        );
