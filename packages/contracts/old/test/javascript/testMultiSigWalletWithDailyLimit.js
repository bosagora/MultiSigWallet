...
contract("MultiSigWalletWithDailyLimit", (accounts) => {
    ...
    it("create multisig", async () => {
        ...
        await new Promise((resolve, reject) =>
            web3.eth.sendTransaction({ to: multisigInstance.address, value: deposit, from: accounts[0] }, (e) =>
                e ? reject(e) : resolve()
            )
        );
        ...
        assert.equal(owner1Balance.add(value1).toString(), (await utils.balanceOf(web3, accounts[0])).toString());
        assert.equal(
        ...
        const transactionId = utils.getParamFromTxEvent(
            await multisigInstance.submitTransaction(multisigInstance.address, 0, dailyLimitEncoded, {
                from: accounts[0],
            }),
            "transactionId",
            null,
        ...
        assert.equal(owner1Balance.add(value2).toString(), (await utils.balanceOf(web3, accounts[0])).toString());
        assert.equal(deposit - value2 - value1, await utils.balanceOf(web3, multisigInstance.address));
        assert.equal(dailyLimitUpdated - value2, await multisigInstance.calcMaxWithdraw());
        await multisigInstance.submitTransaction(accounts[0], value2, "", { from: accounts[1] });
        assert.equal(owner1Balance.add(value2 * 2).toString(), (await utils.balanceOf(web3, accounts[0])).toString());
        assert.equal(deposit - value2 * 2 - value1, await utils.balanceOf(web3, multisigInstance.address));
        assert.equal(dailyLimitUpdated - value2 * 2, await multisigInstance.calcMaxWithdraw());
        ...
        // Let one day pass
        await utils.increaseTimestamp(web3, ONE_DAY + 1);
        assert.equal(dailyLimitUpdated, await multisigInstance.calcMaxWithdraw());
        ...
        assert.equal(owner1Balance, (await utils.balanceOf(web3, accounts[0])).toString());
        assert.equal(deposit - value2 * 3 - value1, await utils.balanceOf(web3, multisigInstance.address));
        assert.equal(dailyLimitUpdated, await multisigInstance.calcMaxWithdraw());
        ...
        assert.equal(owner1Balance.add(value2), (await utils.balanceOf(web3, accounts[0])).toString());
        assert.equal(deposit - value2 * 4 - value1, await utils.balanceOf(web3, multisigInstance.address));
        assert.equal(dailyLimitUpdated - value2, await multisigInstance.calcMaxWithdraw());
